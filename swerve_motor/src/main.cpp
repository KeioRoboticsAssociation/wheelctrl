...
float target_value_table = 0;  // 目標値 rad
float current_value_table = 0;  // 現在値 rad
...
// モーター
Motor motor_wheel(PIN_GATE_DRIVER_ENABLE_WHEEL, PIN_PWM_P_WHEEL, PIN_PWM_N_WHEEL);
Motor motor_table(PIN_GATE_DRIVER_ENABLE_TABLE, PIN_PWM_P_TABLE, PIN_PWM_N_TABLE);

// エンコーダ
Encoder encoder_wheel(PIN_ENA_WHEEL, PIN_ENB_WHEEL);
Encoder encoder_table(PIN_ENA_TABLE, PIN_ENB_TABLE);
...
void Timer_Interrupt_table(void){
  ...
  comm.AttachCurrentTheta(transform_encoder_to_table(current_value_table));
  target_value_table = transform_table_to_encoder(comm.getTargetTheta());
  command_value_table = pid_table.calc_position_command(target_value_table, current_value_table);
